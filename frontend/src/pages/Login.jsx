...
export default function Login({ onLoginComplete, appConfig = {} }) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #1e3a8a 0%, #1e293b 100%)' }}>
      <div style={{ width: '100%', maxWidth: 420, padding: '0 1rem' }}>
        ...
        <div className="card slide-up" style={{ padding: '2rem', boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}>
          <form onSubmit={handleSubmit}>
            ...
            {error && (
              <div style={{ background: '#fef2f2', border: '1px solid #fecaca', color: '#dc2626', borderRadius: '6px', padding: '0.625rem 0.875rem', fontSize: '0.8125rem', marginBottom: '1rem' }}>
                {error}
              </div>
            )}
            <button type="submit" className="btn btn-blue" style={{ width: '100%', padding: '0.75rem', fontSize: '0.9375rem', justifyContent: 'center', borderRadius: '8px' }} disabled={loading}>
          ...
          <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: '1px solid var(--border)' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.375rem' }}>
              {[
                ...
                <button key={a.email} className="tag tag-gray" onClick={() => { setEmail(a.email); setPassword('password'); }} style={{ cursor: 'pointer', border: '1px solid var(--border)', padding: '0.25rem 0.625rem' }}>
                  {a.label}
                </button>
              ))}
            </div>
